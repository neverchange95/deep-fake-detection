...
def put_to_center(img, input_size):
    ...
    image = np.zeros((input_size, input_size, 3), dtype=int)
    start_w = (input_size - img.shape[1]) // 2
    start_h = (input_size - img.shape[0]) // 2
    image[start_h:start_h + img.shape[0], start_w: start_w + img.shape[1], :] = img
    return image

def predict_on_video(face_extractor, video_path, batch_size, input_size, models, device, strategy=np.mean,
...
def predict_on_video_set(face_extractor, videos, input_size, num_workers, test_dir, frames_per_video, models, device,
                         strategy=np.mean,
                         apply_compression=False):
    def process_file(i):
        filename = videos[i]
        y_pred = predict_on_video(face_extractor=face_extractor, video_path=os.path.join(test_dir, filename), device=device,
